...
module.exports = {
   async sendRequest(sqlStr, payload){
    ...
    try {
        ...
        if (res.statusText == 'OK'){
            ...
            let dataType  = Object.fromEntries(res.data.column_meta.map((a,b)=>[a[0],a[1]]))
   ...
   showDatabases(payload){
    ...
    showDatabaseInfo(dbName,payload){
        return this.sendRequest(`SELECT * FROM INFORMATION_SCHEMA.INS_DATABASES WHERE NAME='${dbName}'`, payload)
    },
   testConnect(payload){
   ...
   //添加数据库
   createDatabase(dbName, payload,safe=true,addDBOptions){
        let sqlStr = 'CREATE DATABASE '
        if(safe){
            sqlStr += 'IF NOT EXISTS '
        }
        sqlStr += dbName
        for(let key in addDBOptions) {
            let value = addDBOptions[key];
            if(value) {
                sqlStr += ` ${key} ${value}`
            }
        }
